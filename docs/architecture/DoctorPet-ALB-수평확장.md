...
| 항목 | 내용 |
| --- | --- |
| 문서 버전 | v1.4 (리뷰 반영 — SSE·STOMP fan-out 선행 구현 명시, AZ 분산 배치, deregistration 절차, idle timeout 수치 정정, ALB 도입 시 X-Forwarded-For 2홉 신뢰 체인 보강, XFF 스푸핑 검증 기준 분리, xff_header_processing.mode 명시·real_ip 동작 설명 정정) |
| 작성 기준일 | 2026-08-18 |
...
- **X-Forwarded-For 신뢰 체인이 ALB 도입으로 한 홉 늘어난다(리뷰 지적 P1)** — `docker-compose.yml`의 `AI_RATE_LIMIT_TRUSTED_PROXIES: nginx` 주석에 이미 적혀 있듯, 익명 AI 상담 요청의 rate limit은 `AiClientIpResolver`가 신뢰하는 프록시(현재 nginx 하나)의 `X-Forwarded-For`만 벗겨내 실제 클라이언트 IP를 찾는 구조다. `AiClientIpResolver.resolve()`는 XFF를 오른쪽(마지막 hop)부터 왼쪽으로 훑으면서, 신뢰 목록에 없는 첫 IP를 만나면 그 자리에서 멈춘다. 지금은 "클라이언트 → nginx → app" 1홉이라 nginx만 신뢰 목록에 있으면 정확하다. ALB가 앞에 붙으면 "클라이언트 → ALB → nginx → app" 2홉이 되는데, ALB는 자신이 본 클라이언트 IP를 XFF에 추가해 nginx로 넘기고, nginx는 `proxy_add_x_forwarded_for`로 자기 `$remote_addr`(=ALB의 사설 IP)을 그 뒤에 또 추가한다 — app에 도착하는 XFF는 `..., 실제클라이언트IP, ALB사설IP` 형태가 된다. `AiClientIpResolver`의 신뢰 목록에 ALB IP가 없으면 오른쪽에서 처음 만나는(=신뢰 목록에 없는) 값이 ALB의 사설 IP가 되어 그 자리에서 멈춰버리고, 그 뒤에 있는 실제 클라이언트 IP까지 훑지 못한다 — **결과적으로 ALB를 거친 모든 익명 사용자가 ALB IP 하나로 집계돼 익명 AI rate limit(분당 3회·일 30회)을 서로 나눠 쓰게 된다.** 두 가지 해결 방향이 있고 이 설계는 ①을 권장한다:
  - **① (권장) nginx `ngx_http_realip_module`로 ALB를 신뢰 프록시로 등록** — `set_real_ip_from <ALB가 위치한 서브넷 CIDR>;` + `real_ip_header X-Forwarded-For;` + `real_ip_recursive on;`을 nginx.conf에 추가하면, nginx가 자기 `$remote_addr`를 XFF에서 뽑아낸 실제 클라이언트 IP로 스스로 교체한다. **`real_ip` 모듈은 `$remote_addr` 변수만 바꿀 뿐, 원본 `X-Forwarded-For` 헤더 값 자체를 정리·축소하지는 않는다(리뷰 지적 P2, 정정)** — 클라이언트가 앞쪽에 채워 넣은 값이나 ALB가 붙인 값 모두 `$http_x_forwarded_for`에 그대로 남는다. 이 nginx 설정의 `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`는 그 원본 헤더 뒤에 방금 `real_ip`가 교정한 `$remote_addr`(=실제 클라이언트 IP)를 한 번 더 이어 붙일 뿐이라, app에 도착하는 XFF가 "1홉짜리 체인 하나로 정리"되는 건 아니고 여러 값이 섞인 채로 전달될 수 있다(이전 버전의 이 문서가 그렇게 표현했던 건 정정한다). 그럼에도 안전한 이유는 헤더를 얼마나 "깨끗하게" 만드느냐가 아니라 **어느 값을 신뢰하느냐**에 있다 — `AiClientIpResolver`는 오른쪽(가장 나중에 붙은 값)부터 훑어 신뢰 목록(nginx 하나)에 없는 첫 값에서 멈추므로, 앞쪽에 무엇이 얼마나 섞여 있든 항상 nginx가 방금 새로 붙인 마지막 값(=`real_ip`가 교정한 실제 클라이언트 IP)만 채택한다. `AI_RATE_LIMIT_TRUSTED_PROXIES`는 전혀 손댈 필요가 없다. ALB의 서브넷 CIDR은 3-2절 위 항목에서 확보하는 "ALB용 AZ 2개 이상의 서브넷"과 같은 값이라 실제 구현 시점에 이미 알고 있는 값이다. **신뢰 근거는 헤더 내용이 아니라 TCP 접속 상대다** — `real_ip` 모듈은 "$remote_addr(직접 접속해온 상대, 곧 ALB의 사설 IP)가 신뢰 CIDR 안에 있을 때만" XFF를 근거로 재계산을 하고, 그 값을 헤더에 뭐라고 적었는지는 신뢰 여부와 무관하다. 클라이언트가 ALB로 보내는 요청에 임의의 `X-Forwarded-For: 1.1.1.1, 2.2.2.2`를 미리 채워 넣어도 ALB는 그 값을 지우지 않고 그대로 둔 채 자신이 실제로 관찰한 접속 IP만 맨 뒤에 추가한다 — nginx가 오른쪽부터(=ALB가 가장 마지막에 추가한 값부터) 신뢰 CIDR 밖의 첫 값을 찾아 그것만 `$remote_addr`로 채택하므로, 클라이언트가 앞쪽에 무엇을 써넣든 최종 판단에 전혀 영향을 주지 않는다.
    - **전제: ALB가 실제로 자신이 관찰한 접속 IP를 XFF 맨 뒤에 "추가"한다는 동작에 의존한다(리뷰 지적 P1) — 이 전제가 깨지면 이 설계 전체가 무너진다.** ALB에는 `routing.http.xff_header_processing.mode`라는 로드밸런서 속성이 있고 값은 `append`(기본값) / `preserve` / `remove` 세 가지다. `append`는 클라이언트가 보낸 XFF를 그대로 두고 ALB가 관찰한 실제 접속 IP를 맨 뒤에 추가한다(이 설계가 가정하는 동작). `preserve`로 바뀌면 ALB가 아무것도 추가하지 않고 클라이언트가 보낸 XFF를 그대로 전달하므로, 공격자가 XFF 마지막 값을 직접 제어해 rate limit을 그대로 우회할 수 있다. `remove`로 바뀌면 XFF 헤더 자체가 사라져 `real_ip`가 교정할 근거가 없어지고, `$remote_addr`는 계속 ALB의 사설 IP로 남아 다시 ALB 단위로 요청이 뭉친다. 기본값이 `append`라는 사실에만 기대지 않고, **ALB 생성 시 이 속성을 `append`로 명시적으로 고정**하고(콘솔의 로드밸런서 속성 편집, 또는 IaC 사용 시 해당 속성을 코드로 고정), 배포 검증 항목(아래·§6)에 "이 속성이 실제로 `append`인지" 확인을 포함시킨다.
  - **② (대안) `AiClientIpResolver`가 CIDR 범위를 신뢰 프록시로 지원하도록 코드 확장** — 지금 `AiRateLimitProperties.trustedProxies`는 리터럴 IP·호스트명만 지원해(`List<String>`, `isTrustedProxy()`가 정확히 일치하거나 DNS 재해석한 값과 일치할 때만 신뢰) CIDR을 못 쓴다. 이 대안을 택하면 CIDR 파싱·서브넷 매칭 로직을 새로 추가해야 하는 반면, ①은 인프라 설정만으로 끝난다 — 이 문서(설계 전용, 코드 변경 없음 원칙)와의 정합성을 위해 ①을 기본 권장안으로 삼는다. 이 대안 역시 `routing.http.xff_header_processing.mode`가 `append`여야 한다는 같은 전제를 그대로 공유한다(ALB가 자기 관찰 IP를 안 붙이면 애초에 신뢰할 "ALB가 붙인 값" 자체가 없다).
  - **어느 쪽을 택하든, 검증 항목을 "IP가 분리되는지"로 뭉뚱그리면 안 된다(리뷰 지적 P1) — "서로 다른 실제 클라이언트"와 "같은 클라이언트가 XFF를 조작한 요청"은 정반대 기준으로 검증해야 하는 별개 케이스다.** 네 가지를 분리해서 각각 확인한다:
    - ALB의 `routing.http.xff_header_processing.mode`가 실제로 `append`로 설정돼 있는지 확인한다(콘솔·`aws elbv2 describe-load-balancer-attributes` 등) — 위 전제가 깨지면 아래 검증들은 의미가 없다.
    - 실제 발신지 IP가 다른 두 클라이언트(또는 서로 다른 실제 소스 IP에서 ALB로 보낸 두 요청) → **서로 독립적으로 집계돼야 한다**(한쪽이 429를 받아도 다른 쪽은 영향받지 않음).
    - 같은 클라이언트가 요청마다 임의의 `X-Forwarded-For` 값을 주입 → **반드시 동일 클라이언트로 집계돼야 한다** — 이 케이스가 "분리"로 나오면 rate limit이 헤더 조작으로 우회되는 취약점이므로 실패로 판정한다(공격자가 헤더만 바꿔가며 익명 AI 요청 한도를 무제한으로 늘릴 수 있다는 뜻이다).
    - ALB가 실제로 관찰해 추가한 접속 IP만 최종적으로 신뢰되고, 그 앞의 클라이언트 제공 XFF 값은 실제로 무시되는지(위 ①의 신뢰 근거 설명대로 동작하는지) 직접 확인한다.
...
- [ ] **ALB 로드밸런서 속성 `routing.http.xff_header_processing.mode`를 `append`로 명시적으로 고정**(콘솔 또는 IaC, 기본값이라고 확인 없이 넘어가지 않는다, 3-2절) — 이 값이 `preserve`/`remove`면 아래 nginx `real_ip` 설계 전체가 무너진다.
- [ ] **nginx에 `ngx_http_realip_module`로 ALB 서브넷을 신뢰 프록시로 등록**(`set_real_ip_from`+`real_ip_header X-Forwarded-For`+`real_ip_recursive on`, 3-2절) — 아래 네 가지를 각각 직접 검증(뭉뚱그려 "IP가 분리되는지"만 확인하지 않는다):
  - ALB `routing.http.xff_header_processing.mode`가 실제로 `append`인지 확인(`aws elbv2 describe-load-balancer-attributes` 등).
  - 실제 발신지 IP가 다른 두 클라이언트는 rate limit이 서로 독립적으로 집계된다(한쪽 429가 다른 쪽에 영향 없음).
  - 같은 클라이언트가 `X-Forwarded-For`를 임의로 조작한 요청은 동일 클라이언트로 집계된다(분리되면 실패 — 헤더 조작으로 우회 가능하다는 뜻).
  - ALB가 실제로 관찰해 추가한 접속 IP만 신뢰되고, 클라이언트가 미리 채워 보낸 XFF 값은 무시된다.
