...
>
> v1.68: PR #213 재리뷰 반영 — 전체 삭제가 표시용 `notifications` 행을 지워도 `PAYMENT_PENDING`의 “결제당 1회” 계약은 유지되도록 `notification_delivery_marks`를 추가했다. 새·기존 `dedup_key`를 이 테이블에 보존하고, 구버전 INSERT도 트리거로 동기화해 blue/green 전환 중 이력 누락을 막는다. 삭제 뒤 정산 재실행에도 같은 안내는 다시 저장·SSE 전송되지 않음을 실 MySQL Level 3 테스트로 검증한다.
...
`PAYMENT_PENDING`의 “결제당 1회”는 알림 표시 행의 생존 여부와 무관하다. `notification_delivery_marks.dedup_key UNIQUE(uk_notification_delivery_marks_dedup_key)`가 발행 이력을 영구 보존하고, 표시 행의 `dedup_key`는 구버전 호환·백필용 사본으로만 남긴다. 따라서 `DELETE /api/notifications` 뒤 결제가 계속 `PENDING`이어도 다음 정산 주기는 mark를 발견해 새 알림·SSE를 만들지 않는다. 기존 `notifications.dedup_key`는 새 테이블로 백필하며, blue/green 구버전이 쓰는 새 행은 `trg_notifications_delivery_mark` AFTER INSERT 트리거가 mark에도 기록한다. 적용은 `notification_delivery_marks_v1` 마커와 MySQL `GET_LOCK`으로 직렬화하고, 마커가 있는데 테이블·UNIQUE·트리거가 없으면 부팅을 중단한다.
