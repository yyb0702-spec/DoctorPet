...
>
> v1.67: PR #213 리뷰 반영 — `PAYMENT_PENDING`의 `dedup_key`는 알림 행과 함께 전체 삭제되므로, “결제 확인 중” 안내의 1회 보장은 해당 알림 행이 존재하는 동안(삭제 전까지)으로 한정됨을 §4에 명시했다. 삭제 후 결제가 여전히 `PENDING`이면 다음 정산 주기에 안내가 1회 재발행될 수 있으며, 삭제와 독립적인 발행 이력 보존은 후속 설계 범위다.
...
`PAYMENT_PENDING`의 “결제당 1회”는 삭제되지 않은 같은 알림 행에 한정된다. 이 보장은 `dedup_key`가 해당 행에 저장돼 있기 때문에 성립한다. 따라서 `DELETE /api/notifications`가 알림 행과 키를 함께 지운 뒤 결제가 여전히 `PENDING`이면, 다음 정산 주기에 안내가 1회 재발행될 수 있다. 삭제와 독립된 발행 이력 보존은 현재 범위 밖이다.
