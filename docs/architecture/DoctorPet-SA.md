...
>
> v1.64: 병원 스태프의 회원 진료·결제 이력 조회 API(`GET /api/hospital/reservations/{reservationId}/member-history`)를 §8-6에 추가했다. 예약으로 회원을 해석해 자병원 예약을 활성 결제와 LEFT JOIN·슬롯 시작 시각 최근순으로 페이지 반환하는 읽기 전용 조회이며, 타 병원 예약은 404로 차단한다(새 테이블·락 없음). (병원 결제 목록 API PR #209도 §8-7에서 같은 v1.64를 사용하므로, 나중에 병합되는 쪽이 다음 번호로 renumber한다 — 문서 버전 병행 충돌 관례.)
