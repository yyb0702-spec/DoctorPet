...
> v1.63: PR #198 리뷰 반영 — 현재 적용 시간표와 별도로 미래 발효 시간표 전체를 조회하는 `GET /api/hospital/operating-hours/scheduled`를 §8-6에 추가했다. 화면은 예정 목록 조회가 성공한 뒤에만 저장을 열고, 기존 예정 시간표는 발효일을 명시적으로 선택해야 수정하므로 새로고침·재진입 뒤에도 복구되며 현재 시간표로 같은 발효일을 덮어쓰지 않는다.
...
병원 스태프의 진료시간 API는 현재 적용분과 미래 적용분을 섞지 않는다. `GET /api/hospital/operating-hours`는 `effectiveFrom <= 오늘`인 가장 늦은 한 건만 반환하고, `GET /api/hospital/operating-hours/scheduled`는 `effectiveFrom > 오늘`인 모든 시간표를 발효일 오름차순 배열로 반환한다. 두 API 모두 인증 principal에서 자병원을 판별한다. 화면은 예정 목록 조회가 성공한 뒤에만 저장을 열고, 기존 예정 시간표는 목록에서 발효일을 선택한 경우에만 수정한다. 이로써 새로고침·재진입에도 예정 시간표를 복구하고 현재 시간표를 같은 발효일로 저장해 예정 정책을 덮어쓰는 일을 막는다.
...
| 진료시간 저장 | PUT | /api/hospital/operating-hours | 발효일의 시간표 생성 또는 명시적으로 선택한 예정 시간표 수정 |
