...
>
> v1.67: PR #211에서 병원 결제 목록 API가 병합된 뒤 프런트의 `HOSPITAL_OPS_BACKEND_READY` 플래그를 제거하고 결제 관리 화면·대시보드 미수금 카드를 상시 노출한 구현 상태를 반영했다. 슬롯 관리 화면은 일 단위 임시휴진으로 대체해 제거했다.

> v1.68: PR #213 재리뷰 반영 — 전체 삭제가 표시용 `notifications` 행을 지워도 `PAYMENT_PENDING`의 “결제당 1회” 계약은 유지되도록 `notification_delivery_marks`를 추가했다. 새·기존 `dedup_key`를 이 테이블에 보존하고, BEFORE INSERT 트리거가 구버전 INSERT보다 mark를 먼저 선점해 blue/green 전환 중 삭제된 알림의 재발행까지 차단한다. 삭제 뒤 정산 재실행과 구버전 직접 INSERT 모두 같은 안내를 다시 저장·SSE 전송하지 못함을 실 MySQL Level 3 테스트로 검증한다.
...
| dedup_key | VARCHAR NULL UNIQUE | 구버전 호환·기존 이력 백필용 표시 행 키. 영속 멱등 보장은 `notification_delivery_marks`가 담당 |
...
`PAYMENT_PENDING`의 “결제당 1회”는 알림 표시 행의 생존 여부와 무관하다. 앞 문단의 `uk_notifications_dedup_key` 설명은 기존 표시 행만으로 멱등을 지키던 경로다. PR #213 이후 영속 멱등의 정본은 `notification_delivery_marks.dedup_key UNIQUE(uk_notification_delivery_marks_dedup_key)`이며, 표시 행의 `dedup_key`는 구버전 호환·백필용 사본으로만 남긴다. 따라서 `DELETE /api/notifications` 뒤 결제가 계속 `PENDING`이어도 다음 정산 주기는 mark를 발견해 새 알림·SSE를 만들지 않는다. 기존 `notifications.dedup_key`는 새 테이블로 백필한다. blue/green 구버전이 쓰는 새 행은 `trg_notifications_delivery_mark_before_insert` BEFORE INSERT 트리거가 mark를 먼저 INSERT하며, 이미 mark가 있으면 UNIQUE 위반으로 표시 행 INSERT까지 중단한다. 적용은 `notification_delivery_marks_v2` 마커와 MySQL `GET_LOCK`으로 직렬화하며, 기존 v1 AFTER 트리거는 새 BEFORE 트리거가 설치된 뒤 제거한다. 마커가 있는데 테이블·UNIQUE·BEFORE 트리거가 없으면 부팅을 중단한다.

### notification_delivery_marks

| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| id | BIGINT PK | |
| dedup_key | VARCHAR NOT NULL UNIQUE | 멱등 발행 이력 키(`type:resource_type:resource_id:member_id`) |
| created_at | DATETIME NOT NULL | 최초 발행 시각 |

표시용 알림과 독립된 발행 이력 테이블이다. `PAYMENT_PENDING` 저장은 mark INSERT와 notifications INSERT를 같은 `REQUIRES_NEW` 트랜잭션으로 처리해 둘 중 하나라도 실패하면 모두 롤백한다. 전체 삭제는 notifications만 지우며 mark를 지우지 않는다.
...
병원 스태프의 진료시간 API는 현재 적용분과 미래 적용분을 섞지 않는다. `GET /api/hospital/operating-hours`는 `effectiveFrom <= 오늘`인 가장 늦은 한 건만 반환하고, `GET /api/hospital/operating-hours/scheduled`는 `effectiveFrom > 오늘`인 모든 시간표를 발효일 오름차순 배열로 반환한다. 두 API 모두 인증 principal에서 자병원을 판별한다. 응답은 `scheduleId`와 `updatedAt`을 함께 돌려준다. `PUT /api/hospital/operating-hours`는 `saveMode=CREATE` 또는 `UPDATE`를 명시한다. 병원 행 `FOR UPDATE` 뒤 대상 시간표는 `PESSIMISTIC_WRITE` current read로 읽어, 그보다 앞선 일반 조회가 만든 MySQL `REPEATABLE_READ` 스냅샷을 재사용하지 않는다. `CREATE`는 확정 발효일에 이미 시간표가 있으면 `HOSPITAL_016`(409)으로 거절한다. `UPDATE`는 본문을 바꾸기 전에 목록에서 받은 `targetScheduleId`·`expectedUpdatedAt`으로 `WHERE id AND updated_at` 조건부 UPDATE를 실행해 더 큰 마이크로초 토큰을 선점하고, 갱신 행이 정확히 1건일 때만 최신 행을 다시 적재해 수정한다. 따라서 오래된 요청은 본문 UPDATE 전에 409으로 끝나며, 저장 과정의 감사 시각 갱신 뒤에도 같은 조건부 UPDATE로 토큰 전진을 보장한다. 발행창 안 예약을 피해 발효일을 뒤로 미는 자동 조정은 `CREATE`에만 적용하고, `UPDATE`는 선택한 시간표의 발효일을 그대로 둔다(밀면 대상 시간표를 잃어 정상 편집이 오거부된다). `updatedAt` 비교는 컬럼 정밀도(`datetime(6)`)에 맞아야 하므로 저장 응답의 `updatedAt`은 flush 후 DB 재적재 값으로 돌려준다. 두 경로는 같은 병원 행 `FOR UPDATE` 아래에서 판정하므로 먼저 커밋한 요청만 성립하고, 고정 Clock·동일 시각에도 늦은 요청은 `HOSPITAL_016`(409)으로 끝난다. 화면은 예정 목록 조회가 성공한 뒤에만 저장을 열며 미저장 편집 중에는 다른 시간표 선택과 배경 재조회에 따른 폼 재초기화를 막는다. 이로써 새로고침·재진입에도 예정 시간표를 복구하고 현재 시간표를 같은 발효일로 저장하거나 오래된 화면이 예정 정책을 덮어쓰는 일을 막는다.

| 명칭 | Method | Path | 상태 전이 |
| --- | --- | --- | --- |
| 현재 진료시간 조회 | GET | /api/hospital/operating-hours | 자병원의 오늘 적용 시간표 1건 |
| 적용 예정 진료시간 조회 | GET | /api/hospital/operating-hours/scheduled | 자병원의 미래 시간표 전체, 발효일 오름차순 |
| 진료시간 저장 | PUT | /api/hospital/operating-hours | `CREATE` 생성 또는 선택한 `scheduleId`·`updatedAt` 토큰을 확인한 `UPDATE`; 충돌 시 `HOSPITAL_016`(409) |

### 8-7. 결제

| 명칭 | Method | Path | 권한 |
| --- | --- | --- | --- |
| 빌링키 발급 시도 | POST | /api/payment-methods/billing-key-issues | 보호자 |
| 빌링키 모바일 콜백 | GET | /api/payment-methods/billing-key-issues/{issueId}/callback | PortOne (1회성 검증) |
| 빌링키 iframe 완료 | POST | /api/payment-methods/billing-key-issues/{issueId}/complete | 보호자 |
