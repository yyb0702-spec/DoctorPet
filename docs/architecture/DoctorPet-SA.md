...
전국 데이터 검색 성능 측정 후 기본 이름순 목록에 `(name, id)`, 좌표 바운딩박스에 `(coord_x, coord_y)` 인덱스를 적용한다. `(business_status)` 단일 인덱스는 낮은 선택도와 `name, id` 정렬 미지원으로 OFF/ON 통제 비교에서 개선 효과가 확인되지 않아 제외한다.
