...
- 병원 스태프 진료시간은 현재 적용 시간표(`GET /api/hospital/operating-hours`)와 미래 시간표 전체(`GET /api/hospital/operating-hours/scheduled`)를 분리해 조회한다. 미래 시간표 수정은 목록에서 받은 `scheduleId`·`updatedAt` 토큰을 `UPDATE` 요청에 되보내고, 새 시간표는 `CREATE`로만 저장한다. 병원 행 잠금 뒤 토큰 또는 생성 대상이 달라지면 `HOSPITAL_016`(409)으로 거절해 오래된 화면의 덮어쓰기를 막는다. `UPDATE`는 저장된 `updated_at`을 더 큰 마이크로초 토큰으로 `WHERE id AND updated_at` 조건부 갱신해, 갱신 행이 1건일 때만 성공으로 확정한다.
| 작업 | 주기 | 비고 |
| --- | --- | --- |
