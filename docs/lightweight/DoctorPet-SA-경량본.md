...
| 시스템 설계·ERD·API·상태 머신 | `docs/architecture/DoctorPet-SA.md` v1.64, REST API는 §8 |
...
- 제휴 병원 상세는 최근 90일 완료 응답을 실시간 집계한다. 응답률은 승인·직접 거절·자동 만료 표본 10건 이상, 평균 승인 시간은 승인 10건 이상일 때 각각 정수로 노출하고 기준 미달과 비제휴 병원은 `null`이다. 표본 수는 공개하지 않는다.
- 검색 성능용 인덱스는 전국 단위 데이터 확장 단계에서 실행 계획과 응답시간을 측정한 뒤 적용한다.
- 일반 검색은 `OPEN` 병원만 조회하며 휴업·폐업 병원은 Redis 캐시 HIT/MISS 모두에서 제외한다.
- 전국 공공데이터를 자체 DB에 적재하고 매일 03:00(`Asia/Seoul`)에 갱신한다.
- 최초 데이터 구축에서만 제휴 JSON을 적용하며, 정기 갱신은 공공 원천 기본정보와 영업 상태만 갱신한다. `OPEN → CLOSED_TEMP/CLOSED` 전환 시 `slot.startAt > now`인 `CONFIRMED` 예약만 `HOSPITAL_CANCELED`로 자동 취소한다.
...
- 같은 시점의 활성 예약은 1건만 허용한다. 예약 요청은 병원 행 `FOR UPDATE` 잠금 아래 `OPEN`을 확인하고 그 잠금을 슬롯의 `OPEN → RESERVED` 낙관적 락 점유와 `REQUESTED` 저장까지 유지한다. 요청·승인·공공데이터 갱신은 같은 병원 행 잠금으로 직렬화한다.
- 병원 거절 사유는 직원 부족, 슬롯 등록 오류, 진료 불가, 기타의 4종으로 관리한다.
- 예약 승인 타임아웃 또는 허용 범위 내 취소·거절 시 슬롯을 반환한다. 병원 승인은 현재 영업상태가 `OPEN`일 때만 허용한다.
- 병원 스태프는 소속 병원의 `CONFIRMED` 예약만 사유와 함께 `HOSPITAL_CANCELED`로 취소할 수 있다. 공공데이터에서 병원이 `OPEN → CLOSED_TEMP/CLOSED`로 전환될 때는 대상 조회와 조건부 UPDATE 모두 `slot.startAt > now`인 `CONFIRMED` 예약만 자동 취소한다. 두 경우 모두 슬롯을 `OPEN`으로 반환하고 보호자에게 `RESERVATION_HOSPITAL_CANCELED` 알림을 저장한다. 예약 이력 `cancelCount`는 `CANCELED`와 `HOSPITAL_CANCELED`를 합산한다.
...
- 오청구 전액 환불은 MVP+ 고도화에서 도입했다. 대상은 `PAID`·`BILLING_KEY` 결제이고 사유만 받아 결제 금액을 그대로 취소하며, 선점은 환불 이력 테이블의 `UNIQUE(payment_id)`가 담당한다.
- 결제수단 기본값 지정과 예약 결제수단 재지정은 구현 완료다. 회원별 활성 기본 결제수단은 최대 1건이고, 예약 재지정은 결제 선기록 전·진료 시작 전에만 허용한다.
- 청구 항목은 병원 스태프가 진료 완료 후·청구 선기록 전까지만 작성·수정하고, 선기록 이후에는 수정하지 않는다. 할인·조정은 음수 금액 항목으로 기록하므로 단가·항목 금액은 signed 정수이며 UNSIGNED로 만들지 않는다. 총액은 항목 합계와 일치하고 합계는 0보다 커야 한다.
- 항목 수정과 청구 선기록은 같은 예약 행을 `PESSIMISTIC_WRITE`로 잠가 직렬화하고, 항목 수정은 `WHERE payment_id IS NULL` 조건부로만 반영한다.
- 선기록 순서는 예약 행 락 → 초안 재조회·합계 산출 → `payments` INSERT → 채번된 id로 초안 스탬프로 고정한다. `payments.id`가 IDENTITY라 INSERT가 스탬프보다 먼저여야 하며, 스탬프 건수가 재조회한 초안 수와 다르면 전체를 롤백한다. 총액은 서버가 항목 합계로 산출하며 클라이언트 총액을 신뢰하지 않는다.
- 초안 저장과 청구는 별도 요청이라 그 사이 잠금이 끊긴다. 저장 응답이 준 초안 토큰을 청구가 되보내 서버가 다시 대조하며, 다른 직원이 초안을 바꿨으면 409로 거부한다.
- 항목화 이전 결제는 항목이 없고 백필하지 않는다. 그 결제의 영수증은 항목 없이 총액만 제공한다.
- 초안 쓰기는 그 예약의 활성 결제 상태로 판정한다. 활성 결제가 없거나 `REFUNDED`(정정 재청구 준비)일 때만 허용하고, `PENDING`·`PAID`·`OFFLINE_PAID`·`OFFLINE_REQUIRED`면 거부한다. 전액 환불은 `superseded_at`을 세우지 않으므로 이 예외가 없으면 정정 재청구를 시작할 수 없다.
- 영수증은 `PAID`·`OFFLINE_PAID`·`REFUNDED` 결제에 구조화 JSON으로만 제공한다. 결제 시점 스냅샷과 안정 식별자·예약 시점 펫 스냅샷만 쓰므로 현재 표시명 변경이 과거 영수증을 바꾸지 않는다. 보호자는 본인 예약 결제만, 병원 스태프는 자기 병원 결제만 조회하며 환불 사유는 노출하지 않는다.
...
- 병원 스태프 진료시간은 현재 적용 시간표(`GET /api/hospital/operating-hours`)와 미래 시간표 전체(`GET /api/hospital/operating-hours/scheduled`)를 분리해 조회한다. 미래 시간표 수정은 목록에서 받은 `scheduleId`·`updatedAt` 토큰을 `UPDATE` 요청에 되보내고, 새 시간표는 `CREATE`로만 저장한다. 병원 행 잠금 뒤 대상 시간표는 `PESSIMISTIC_WRITE` current read로 다시 읽어 이전 일반 조회의 MySQL 스냅샷을 쓰지 않는다. `UPDATE`는 본문 변경 전에 `WHERE id AND updated_at` 조건부 갱신으로 토큰을 선점하고, 갱신 행이 1건일 때만 최신 행을 다시 적재해 수정한다. 토큰 또는 생성 대상이 달라지면 `HOSPITAL_016`(409)으로 거절해 오래된 화면의 덮어쓰기를 막는다.
| 작업 | 주기 | 비고 |
| --- | --- | --- |
| 자동 노쇼 판정 | 1분 | +10분 경과 건을 `NO_SHOW_PENDING`, 추가 5분 경과 건을 `NO_SHOW`로 처리 |
| 예약 승인 타임아웃 | 1분 | 응답 기한 경과 건 처리 |
| 결제 상태 정산 | 5분 | 미확정 결제 상태 확인 |
| 슬롯 생성 | 일 배치 | 병원 행 잠금 아래 `OPEN` 병원만 향후 14일치 유지 |
| 공공데이터 주기 갱신 | 매일 03:00 | 전국 원천정보 갱신 및 휴·폐업 전환 병원의 시작 전 확정 예약 자동 취소 |
