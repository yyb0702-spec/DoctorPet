...
자동 노쇼 배치는 한 실행에서 같은 예약을 대기 전이와 최종 전이로 연달아 처리할 수 있다. 따라서 `maxScannedPerRun`은 예약의 고유 개수가 아니라 조회·전이 시도 횟수이고, `processed`는 한 번 이상 상태 전이에 성공한 예약 수로 집계한다. 대기·최종 전이별 실제 건수는 상태 이력과 별도 지표로 확인한다.
## 10. 데이터·보안 원칙
