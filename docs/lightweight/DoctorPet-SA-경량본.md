...
- 최초 데이터 구축에서만 제휴 JSON을 적용하며, 주간 갱신은 공공 원천 기본정보와 영업 상태만 갱신한다. `OPEN → CLOSED_TEMP/CLOSED` 전환 시 `CONFIRMED` 예약은 `HOSPITAL_CANCELED`로 자동 취소한다.
...
- 병원 스태프는 소속 병원의 `CONFIRMED` 예약만 사유와 함께 `HOSPITAL_CANCELED`로 취소할 수 있다. 공공데이터에서 병원이 `OPEN → CLOSED_TEMP/CLOSED`로 전환될 때도 시스템이 `CONFIRMED` 예약만 같은 상태로 자동 취소한다. 두 경우 모두 슬롯을 `OPEN`으로 반환하고 보호자에게 `RESERVATION_HOSPITAL_CANCELED` 알림을 저장한다.
...
| 공공데이터 주기 갱신 | 매주 월요일 03:00 | 전국 원천정보 갱신 및 휴·폐업 전환 병원의 확정 예약 자동 취소 |
