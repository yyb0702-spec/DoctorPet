...
- 항목 수정과 청구 선기록은 같은 예약 행을 `PESSIMISTIC_WRITE`로 잠가 직렬화하고, 항목 수정은 `WHERE payment_id IS NULL` 조건부로만 반영한다. 총액은 서버가 항목 합계로 산출하며 클라이언트 총액을 신뢰하지 않는다.
- 항목화 이전 결제는 항목이 없고 백필하지 않는다. 그 결제의 영수증은 항목 없이 총액만 제공한다.
- 영수증은 `PAID`·`OFFLINE_PAID`·`REFUNDED` 결제에 구조화 JSON으로만 제공한다. 보호자는 본인 예약 결제만, 병원 스태프는 자기 병원 결제만 조회하며 환불 사유는 노출하지 않는다.
- 보호자 셀프 재청구는 `OFFLINE_REQUIRED`에서만 허용하고, `PENDING`에서는 이중 결제 위험 때문에 허용하지 않는다(정산 스케줄러가 확정한다).
