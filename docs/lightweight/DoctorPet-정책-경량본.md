...
- 예약은 `REQUESTED`로 생성하며 영업상태가 `OPEN`인 병원이 승인하면 `CONFIRMED`, 거절하면 `REJECTED`가 된다. 승인 전에 휴업·폐업으로 변경되면 승인할 수 없다.
...
- 병원 스태프는 소속 병원의 `CONFIRMED` 예약만 공백이 아닌 255자 이하 사유와 함께 `HOSPITAL_CANCELED`로 취소할 수 있다. 공공데이터에서 병원이 `OPEN → CLOSED_TEMP/CLOSED`로 전환될 때도 시스템이 `CONFIRMED` 예약만 자동 취소한다. 취소 시 슬롯을 반환하고 보호자에게 `RESERVATION_HOSPITAL_CANCELED` 알림을 저장한다.
### 진료
```plain text
