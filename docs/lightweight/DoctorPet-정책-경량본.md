...
- 영수증은 `PAID`·`OFFLINE_PAID`·`REFUNDED` 결제에 구조화 JSON으로 제공하고, 보호자는 본인 결제만·병원 스태프는 자기 병원 결제만 조회한다. 환불 사유는 감사용이라 보호자에게 노출하지 않는다.
- 보호자 셀프 재청구는 `OFFLINE_REQUIRED`에서만 허용한다. `PENDING`에서는 허용하지 않고 정산 스케줄러의 확정을 기다린다.
