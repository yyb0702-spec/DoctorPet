...
1. **요구사항·제외 범위**: Issue의 목표·Acceptance Criteria 대비 누락이 없는지, 제외 범위를 침범하지 않았는지. 요구사항을 임의로 축소·변경하지 않았는지, 미해결 질문(Open Questions)이 무시되지 않았는지.
2. **테스트 근거**: 변경된 동작에 맞는 테스트와 회귀 방지 근거가 있는지. STRICT 변경인데 **신호별 필수 테스트**가 없으면 반환 — 락·트랜잭션 관련은 동시성 테스트, 그 외 DDL(조회용 인덱스 등)은 Level 3 통합 검증(AGENTS.md STRICT 신호 목록의 요구를 따른다). Level 5·6 검증에는 서버 로그 확인 결과(예상 못한 500·미처리 예외 없음)가 포함됐는지.
...
4. **문서 갱신**: API·정책·스키마가 바뀌었는데 SA·관련 문서가 안 바뀌었는지. 정본을 바꿨으면 경량본(`docs/lightweight/`)은 **같은 PR에서 갱신**됐는지, Wiki는 갱신됐거나 후속 이슈가 남았는지(CI 밖이라 후속 이슈 허용).
5. **과한 추상화·범위 초과**: Generic Manager, 숨은 공통 계층, 요구사항과 무관한 리팩토링.
6. **산출물 품질**: 근거 없는 일반론, 실행하지 않은 검증을 완료로 적은 주장, 동작을 설명하지 못하는 테스트명. 상세 패턴은 [AI 슬롭 제거](remove-ai-slop.md)를 따른다.
