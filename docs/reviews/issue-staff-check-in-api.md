...
`NO_SHOW_PENDING` 추가 유예가 끝나는 시점에는 `HospitalReservationApplicationService.checkIn()`과 `ReservationNoShowProcessor.process()`가 각각 상태 조건부 UPDATE를 수행한다. 실제 서비스 경합 테스트에서 체크인 승리 또는 최종 노쇼 승리 중 하나만 성립하고, 승자에 해당하는 이력·알림만 남는 것을 검증한다.
...
- [x] 기능이 요구사항대로 동작한다
- [x] 필요한 테스트 작성 및 통과 (docs/testing/verification-guide.md 양식으로 기록)
- [x] 관련 문서 갱신
- [ ] 범위 밖 변경이 없다
...
- [x] 직원이 소속 병원의 예약에 대해서만 도착 확인할 수 있다
- [x] 중복 요청에도 도착 이력과 상태 이력이 중복 생성되지 않는다
- [x] 도착 확인된 예약이 자동 노쇼로 변경되지 않는다
- [x] 진료·결제 완료 예약이 자동 노쇼로 변경되지 않는다
- [x] API와 스케줄러의 동시 실행에서 `CHECKED_IN` 또는 `NO_SHOW` 중 유효한 전이 하나만 성립한다
- [x] 최종 노쇼 확정 전 환자에게 노쇼 확정 알림이 발송되지 않는다
- [x] Level 5 로컬 기동 검증 결과를 기록한다
- [ ] Level 6 실제 HTTP 검증 결과를 기록한다
- [x] 예약 상태 머신 변경 시 SA를 갱신한다
