...
2. 현재 시각이 `예약 슬롯 시작 시각 + 10분` 이상이다.
3. 아직 체크인·취소·진료 완료 등 다른 처리가 되지 않았다.

예를 들어 예약 시작이 오전 10시라면 오전 10시 9분에는 아무 일도 하지 않고, 오전 10시 10분부터 자동 노쇼 대상이 된다.
...
실행 주기, 첫 실행 지연, 타임존, 10분 유예 시간, 페이지 크기, 실행당 최대 조회 수, 재시도 횟수, DB 잠금 대기 시간을 환경 변수로 조정할 수 있다. 기본 타임존은 `Asia/Seoul`이다.
...
- 시작 시각 +10분 전에는 처리하지 않고, 정확히 +10분부터 처리한다.
- `CONFIRMED` 예약만 처리한다.
