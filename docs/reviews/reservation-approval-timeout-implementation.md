...
한 번의 실행에서는 일반 만료 예약과 백오프가 끝난 재시도 예약을 별도 커서로 조회한다. 기본 처리량의 80%는 일반 만료, 20%는 재시도에 우선 배정하고, 한쪽 대상이 부족하면 남은 처리량을 다른 쪽에 넘긴다. 따라서 실패 예약이 많이 쌓여도 일반 만료 예약이 무기한 밀리지 않고, 일반 예약이 많아도 재시도 예약이 계속 처리된다.
