...
- 정정 재청구: **확정 정책이며, 기존의 "환불한 예약은 재청구할 수 없다" 결정을 대체한다**(SA §9-4). 금액 정정은 기존 `PAID` 결제를 전액 환불한 뒤 **새 `payments` 행을 만들고 `correction_of`로 이전 결제에 연결**하는 방식만 쓴다. 예약당 활성 결제는 1건만 허용하고, 환불·대체된 과거 결제는 지우지 않고 이력으로 남긴다. `payments.UNIQUE(reservation_id)`가 이 흐름을 막으므로 **"활성 결제 UNIQUE"로 대체**했다(SA §4 payments — 생성 컬럼 `active_reservation_id` + `UNIQUE`, 구 `UNIQUE(reservation_id)` 제거). **구현 완료** — 셀프 복구·정정 재청구 모두 조건부 `supersede` UPDATE로 원 결제를 비활성화한 뒤 새 결제를 만들고, 청구 선기록·초안 게이트 두 곳을 활성 결제 기준으로 교체했다(고도화 `docs/enhancement/결제.md` 3.3·3.5-a). **부분 환불은 재청구와 무관한 별개 계약이며 계속 미확정·제외**다(`docs/enhancement/결제.md` 3.5-b)
- 청구 항목화(PR #158 구현 완료): 청구 항목(`payment_items`)은 **병원 스태프가 진료 완료 후·청구 선기록 전까지만** 작성·수정하고, 선기록(Payment `PENDING` 생성) 이후에는 절대 수정하지 않는다. 항목은 청구 시점 스냅샷이며, 기존 결제의 금액 정정은 항목 수정이 아니라 정정 재청구 절차만 쓴다 — 이 금지는 데이터 정합성뿐 아니라 **정정 재청구를 우회 구현하지 못하게 하는 경계**다(SA §9-4). 그래서 금지는 문서가 아니라 **직렬화로 강제한다** — 항목 수정과 청구 선기록은 같은 예약 행을 `PESSIMISTIC_WRITE`로 잠그고(청구가 이미 쓰는 락), 항목 수정은 `WHERE payment_id IS NULL` 조건부 UPDATE로만 반영한다(STRICT, SA §4·§9-4). **총액은 서버가 항목 합계로 산출**하며 클라이언트 총액을 신뢰하지 않는다. 할인·조정은 별도 할인 필드가 아니라 **음수 금액 항목**으로 기록하므로 `quantity`는 양수(요청 DTO `@Positive`), `unit_price`·`amount`는 음수를 허용하는 signed 정수이며 **UNSIGNED로 만들지 않는다**. 최종 합계는 0보다 커야 한다. 항목화 이전 결제는 항목이 없고 **백필하지 않는다**. 세율·부가세 분리와 진료 항목 마스터 표준화는 제외
- 영수증(PR #158 구현 완료): `PAID`·`OFFLINE_PAID`·`REFUNDED` 결제에 **구조화 JSON만** 제공한다(PDF·전자문서·세금계산서·현금영수증 국세청 연동·진료확인서 제외). 보호자는 본인 예약 결제만, 병원 스태프는 자기 병원 결제만 조회한다. 결제 항목·총액·결제 일시·카드 정보는 결제 시점 스냅샷, 병원·보호자는 안정 식별자, 펫은 예약 시점 이름·종 스냅샷을 사용하며 변경 가능한 표시명·연락처는 넣지 않는다. 환불 사유는 감사용이라 보호자 알림·응답에 노출하지 않는다(SA §4 `payment_refunds.reason` 기존 결정 승계)
- 결제 실패 셀프 복구: `Payment.status == OFFLINE_REQUIRED`에서만 허용하고, 보호자가 ACTIVE 결제수단을 새로 지정한 뒤 **명시적으로 재청구를 요청**해야 한다. `PENDING`에서는 어떤 셀프 재청구도 허용하지 않는다 — 승인 여부가 불확정이라 이중 결제 위험이 있어 정산 스케줄러의 단건 조회 결과를 기다린다(SA §9-7). 새 결제는 `recovery_of`로 원 결제를 연결하고, 원 항목을 보존한 새 항목 행과 원 총액을 같은 선기록 트랜잭션에서 그대로 승계한다. 복제 항목 합계가 원 총액과 다르면 대체를 롤백하고 PG 승인 없이 운영자 확인 대상으로 남긴다. 항목화 이전 원 결제는 가짜 항목 없이 총액만 승계한다. **셀프 재청구와 오프라인 정산은 승자 규칙을 DB로 못박는다** — 둘 다 `superseded_at IS NULL`(활성)을 전제로 한 조건부 UPDATE로만 성립해 먼저 커밋한 쪽이 이기고 늦은 쪽은 409다(STRICT: 자동결제·현장 수납 이중 수납 방지, SA §5-2·§9-4)
