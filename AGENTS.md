...
- 영수증: `PAID`·`OFFLINE_PAID`·`REFUNDED` 결제에 **구조화 JSON만** 제공한다(PDF·전자문서·세금계산서·현금영수증 국세청 연동·진료확인서 제외). 보호자는 본인 예약 결제만, 병원 스태프는 자기 병원 결제만 조회한다. 결제 항목·총액·결제 일시·카드 정보는 결제 시점 스냅샷, 병원·보호자는 안정 식별자, 펫은 예약 시점 이름·종 스냅샷을 사용하며 변경 가능한 표시명·연락처는 넣지 않는다. 환불 사유는 감사용이라 보호자 알림·응답에 노출하지 않는다(SA §4 `payment_refunds.reason` 기존 결정 승계)
