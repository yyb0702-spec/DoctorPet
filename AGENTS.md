...
- 정정 재청구: **확정 정책이며, 기존의 "환불한 예약은 재청구할 수 없다" 결정을 대체한다**(SA §9-4). 금액 정정은 기존 `PAID` 결제를 전액 환불한 뒤 **새 `payments` 행을 만들고 `correction_of`로 이전 결제에 연결**하는 방식만 쓴다. 예약당 활성 결제는 1건만 허용하고, 환불·대체된 과거 결제는 지우지 않고 이력으로 남긴다. `payments.UNIQUE(reservation_id)`가 이 흐름을 막으므로 **"활성 결제 UNIQUE"로 대체**하며(SA §4 payments), 그 스키마·코드 변경이 끝나기 전까지는 코드상 재청구가 여전히 불가하다 — 구현 PR이 제약 교체까지 함께 수행한다. **부분 환불은 재청구와 무관한 별개 계약이며 계속 미확정·제외**다(`docs/enhancement/결제.md` 3.5-b)
- 청구 항목화: 청구 항목(`payment_items`)은 **병원 스태프가 진료 완료 후·청구 선기록 전까지만** 작성·수정하고, 선기록(Payment `PENDING` 생성) 이후에는 절대 수정하지 않는다. 항목은 청구 시점 스냅샷이며, 기존 결제의 금액 정정은 항목 수정이 아니라 정정 재청구 절차만 쓴다 — 이 금지는 데이터 정합성뿐 아니라 **정정 재청구를 우회 구현하지 못하게 하는 경계**다(SA §9-4). 할인·조정은 별도 할인 필드가 아니라 **음수 금액 항목**으로 기록하므로 `quantity`는 양수, `unit_price`·`amount`는 음수를 허용하는 signed 정수이며 **UNSIGNED로 만들지 않는다**. 총액은 항목 합계와 일치하고 최종 합계는 0보다 커야 한다. 세율·부가세 분리와 진료 항목 마스터 표준화는 제외
...
- 결제 실패 셀프 복구: `Payment.status == OFFLINE_REQUIRED`에서만 허용하고, 보호자가 ACTIVE 결제수단을 새로 지정한 뒤 **명시적으로 재청구를 요청**해야 한다. `PENDING`에서는 어떤 셀프 재청구도 허용하지 않는다 — 승인 여부가 불확정이라 이중 결제 위험이 있어 정산 스케줄러의 단건 조회 결과를 기다린다(SA §9-7)
