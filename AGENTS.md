...
- 실시간 알림·채팅: 예약·결제 알림은 MVP2 단방향 SSE를 유지한다(#40). `NotificationPusher` 추상화 + `SseNotificationPusher` 구현, 티켓 기반 인증·커밋 후 전송 계약은 변경하지 않는다. 병원↔회원의 예약당 채팅은 native WebSocket + STOMP를 사용하며, STOMP CONNECT `Authorization` 헤더의 JWT를 검증한다. 채팅은 단일 인스턴스 Spring SimpleBroker를 기준으로 하고 다중 인스턴스 fan-out은 후속 범위이며, 메시지는 저장 커밋 후에만 전달한다(SA §9-8·§9-12)
