...
public interface PaymentNotificationPublisher {
     ...
     * 호출하지 않는다. 구현은 <b>알림 행이 존재하는 동안(전체 삭제 전까지) 결제당 정확히 1회</b>만 저장한다. 전체 삭제 후
     * 결제가 여전히 PENDING이면 다음 정산 주기에 안내가 1회 재발행될 수 있다. 반복 사이클은
     * 존재조회로 값싸게 걸러내고, 락 밖 경로(웹훅 단건 트리거)가 배치와 동시에 처리하는 경합은 알림의 {@code dedup_key}
     * UNIQUE 제약으로 원자적으로 1건만 저장되게 막는다(동시 삽입 중 진 트랜잭션은 이미 발행된 것으로 흡수). 유형별
