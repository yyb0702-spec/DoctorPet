 ...
 *
 * <p>수신자는 recipientType(MEMBER|HOSPITAL) + recipientId로 식별한다(고도화 3.10). 전송은 알림 저장 트랜잭션이
 * <b>커밋된 이후</b>에만 호출한다({@link NotificationPushListener}). 저장이 알림의 원본이고 전송은 부가 채널이므로,
 * 전송 실패는 저장·상위 도메인 트랜잭션에 영향을 주지 않는다.
 */
