...
public class ReservationNoShowLock {

    private static final String LOCK_NAME = "doctorpet:reservation-auto-no-show";
    ...
    public <T> Optional<T> executeIfAcquired(int waitSeconds, Supplier<T> action) {
        return jdbcTemplate.execute((ConnectionCallback<Optional<T>>) connection -> {
            if (!acquire(connection, waitSeconds)) return Optional.empty();
            try {
    ...
    private void release(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("select release_lock(?)")) {
            ...
            log.warn("예약 자동 노쇼 DB 잠금 해제 실패", exception);
        }
    }
}
