...
/**
 * 예약 상태 변경을 알림으로 전달하는 도메인 포트(SA §9-8, 이슈 #88·#166).
 *
 * <p>대상 이벤트는 보호자 수신(승인 CONFIRMED·거절 REJECTED: 병원 수동/승인 타임아웃 자동·노쇼 NO_SHOW)과
 * 병원 수신(새 예약 요청 접수 REQUESTED, #166)이다. 저장은 상태 전이 트랜잭션 안에서 이뤄져 전이가
 * 롤백되면 알림도 남지 않는다 — 이 계약이 이 포트의 범위다(보호자·병원 수신 모두 동일).
 *
...
public interface ReservationNotificationPublisher {
    ...
    /**
     * 보호자가 새 예약을 요청해 병원이 확인해야 할 REQUESTED 예약이 생겼다(#166).
     *
     * <p>유일한 병원 수신(HOSPITAL) 알림이다 — 수신자는 회원이 아니라 병원이고, 저장·읽음은 병원 단위로
     * 공유된다(고도화 알림 3.10). 일반 예약 요청과 대기열 승급 수락 두 경로 모두 이 메서드를 쓴다.
     */
    void publishReservationRequested(Long hospitalId, Long reservationId);
