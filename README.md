...
### 3. 가용성 이중화 — 설계는 하고, 구축은 기준까지 보류

가용성 이중화(다중 인스턴스·자동 장애 전환)를 **설계까지 마친 뒤 구축은 보류**했습니다. 지금 구축했을 때 얻는 이점과 드는 비용, 현재 성능·장애 위험을 같은 표에서 비교해 내린 결정입니다.

| 항목 | 내용 |
| --- | --- |
| 현재 성능 | P95 300ms · 100 RPS 충족 |
| 지금 구축 시 이점 | 장애 자동 전환 · 운영 복구 단순 |
| 이중화 상시 비용 | ALB + EC2 1대 · 월 약 $30~40 증가 · 다중 인스턴스 대응 필요 |
| 결정 | **설계 완료 · 구축 보류** |

- **보류 근거**: ① 현재 트래픽이 성공 기준을 충족 ② 선제 확장의 실측 근거 부족 ③ 이중화의 전제인 RDS·ElastiCache 컷오버는 검증 완료. (캐시는 Redis OSS 대비 최대 20% 저렴한 **Valkey** 선택)
- **보류 위험**: Single-AZ 수동 복구, Redis 교체 시 인증 정보 일부 유실 가능.
- **재검토 조건**: ① 다운타임 발생 ② SLA 요구 ③ P95 300ms 또는 100 RPS 초과.

이 보류는 기능 실패가 아니라 **사용자 요청을 막지 않는 다음 확장 과제로 분리한 것**이며, 이중화를 재검토하면 실제 트래픽 전 ALB 검증과 SSE/STOMP 다중 인스턴스 fan-out을 우선 구현합니다.
...
| [트러블슈팅 기록 (Wiki)](https://github.com/yyb0702-spec/DoctorPet/wiki/Troubleshooting) | 도메인별로 실제 겪은 함정 기록 (이중 청구·알림 저장 실패·토큰 만료 등) |
