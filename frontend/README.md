...
결제수단·결제 내역, 알림·SSE, 채팅, 병원 스태프의 예약 관리와 예약 상세 청구·오프라인 정산·환불 화면을 백엔드 API에 연결한다. 병원 스태프의 결제 목록 대시보드와 슬롯 관리 화면은 백엔드 API가 없어 비활성화돼 있다.
...
- **병원 스태프 결제 목록·슬롯 관리**: 화면 코드는 준비돼 있지만 `HOSPITAL_OPS_BACKEND_READY=false`로 메뉴와 `/staff/payments`·`/staff/slots` 라우트가 비활성화돼 있다. `GET /api/hospital/payments`와 `GET/PATCH /api/hospital/slots*`가 구현된 뒤 기능 플래그를 활성화해야 한다. 예약 상세의 청구·오프라인 정산·환불은 현재 실연동 범위다.
